...
class admin_interface:
    ...
    def hal_beranda_admin(self):
        ...
        if Order.antrean:
            ...
            for idx, order in enumerate(Order.antrean, start=1):
                ...
                tk.Button(frame_order_history, text="Lihat", command=lambda o=order: self.popup_detail_pesanan(o)).grid(row=idx, column=4, padx=5, pady=2, sticky='nsew')
        else:
        ...
        # Tombol Refresh
        tk.Button(frame_dashboard, text="Refresh", command=self.hal_beranda_admin).grid(row=len(Order.antrean) + 6, column=0, columnspan=2, pady=10)
        
        # Tombol Riwayat Pesanan
        tk.Button(frame_dashboard, text="Riwayat Pesanan", command=self.hal_riwayat_pesanan).grid(row=len(Order.antrean) + 7, column=0, columnspan=2, pady=10)

        # Tombol Keluar
        tk.Button(frame_dashboard, text="Keluar", command=self.keluar_admin).grid(row=len(Order.antrean) + 8, column=0, columnspan=2, pady=10)
    ...
    def hal_riwayat_pesanan(self):
        ...
        for order in Order.riwayat.values():
            total_item += order.cek_jumlah()
            total_pendapatan += order.cek_total()
        ...
        tk.Label(frame_dashboard, text=f"Total Pesanan: {Order.counter} pesanan", font=(self.font_family, self.fsize_n), anchor='w', justify="left").grid(row=2, column=0, columnspan=4, sticky='nsew', padx= 10)
        tk.Label(frame_dashboard, text=f"Total Item: {total_item}", font=(self.font_family, self.fsize_n), anchor='w', justify="left").grid(row=3, column=0, columnspan=4, sticky='nsew', padx= 10)
        tk.Label(frame_dashboard, text=f"Total Pendapatan: Rp {total_pendapatan:,.2f}", font=(self.font_family, self.fsize_n), anchor='w', justify="left").grid(row=4, column=0, columnspan=4, sticky='nsew', padx= 10)
        ...
        if Order.riwayat:
            ...
            tk.Label(frame_order_history, text="Status", font=(self.font_family, 12)).grid(row=0, column=2, padx=5, pady=5, sticky='nsew')
            tk.Label(frame_order_history, text="Total", font=(self.font_family, 12)).grid(row=0, column=3, padx=5, pady=5, sticky='nsew')
            tk.Label(frame_order_history, text="Struk Pesanan", font=(self.font_family, 12)).grid(row=0, column=4, padx=5, pady=5, sticky='nsew')
            ...
            for idx, order in enumerate(Order.riwayat.values(), start=1):
                ...
                tk.Label(frame_order_history, text=f"{order.status.value}").grid(row=idx, column=2, padx=5, pady=2, sticky='nsew')
                tk.Label(frame_order_history, text=f"Rp {order.cek_total():,.2f}").grid(row=idx, column=3, padx=5, sticky='nsew')
                tk.Button(frame_order_history, text="Struk", command=lambda o=order: self.popup_detail_pesanan(o)).grid(row=idx, column=4, padx=5, pady=2, sticky='nsew')
        else:
            # Jika belum ada riwayat pesanan
            tk.Label(frame_dashboard, text="Belum ada pesanan masuk.").grid(row=7, column=0, columnspan=4, pady=5)
        
        # Tombol Refresh
        tk.Button(frame_dashboard, text="Refresh", command=self.hal_riwayat_pesanan).grid(row=8, column=0, columnspan=2, pady=10)
        
        # Tombol Keluar
        tk.Button(frame_dashboard, text="Kembali", command=self.hal_beranda_admin).grid(row=9, column=0, columnspan=2, pady=10)
    ...
    def popup_detail_pesanan(self, order: Order):
        ...
        ttk.Button(button_frame, text="Proses Pesanan", command=lambda: [popup.destroy(), self.hal_proses_pesanan(order)]).grid(row=0, column=0, padx=10, pady=5)
    ...
    def hal_proses_pesanan(self, order: Order):
        ...
        tk.Label(frame_detail, text=f"{order}", font=(self.font_family, self.fsize_n), anchor="e").grid(row=4, column=0, sticky='w', padx=10, pady=2)
    ...
    def kembali(self, order: Order):
        ...
        if order.status == Status.IN_PROGRESS:
            if messagebox.askokcancel("Konfirmasi", "Apakah Anda yakin ingin kembali? Pesanan ini akan tetap ditandai sebagai DIPROSES sehingga pemesan tidak dapat mengubahnya lagi."):
                self.hal_beranda_admin()
        else:
            self.hal_beranda_admin()
